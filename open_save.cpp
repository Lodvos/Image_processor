...
void Image::Read(const char *path) {
    ...
    width_ = (info_header[4]) + (info_header[5] << 8) + (info_header[6] << 16) + (info_header[7] << 24);     // NOLINT
    height_ = (info_header[8]) + (info_header[9] << 8) + (info_header[10] << 16) + (info_header[11] << 24);  // NOLINT
    x_pixels_per_m_ =
